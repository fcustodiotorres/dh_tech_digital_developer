...
const emailReference = document.querySelector("#email-input");
const passwordReference = document.querySelector("#password-input");
const baseUsuarios = baseDeDados.usuarios;
const btnLogin = document.querySelector("button");
const loaderReference = document.querySelector("#loader");

btnLogin.addEventListener("click", () => {
  for (let i = 0; i < baseUsuarios.length; i++) {
    if (emailReference.value === baseUsuarios[i].email) {
      if ((passwordReference.value === baseUsuarios[i].password)) {
        loaderReference.classList.remove('hidden')
      }
    }
  }
});
